...
export default function StatusBadge({ candidate }: StatusBadgeProps) {
  ...
  if (candidate.status === 'signed') {
    return (
      <span className="rounded-md bg-green px-3 py-1 text-xs font-bold text-white">
        已簽署 {signedCount} 項
      </span>
    );
  }
  ...
  if (candidate.status === 'met') {
    return <span className="rounded-md bg-sky-pale px-3 py-1 text-xs font-bold text-ink/80">已拜會</span>;
  }
